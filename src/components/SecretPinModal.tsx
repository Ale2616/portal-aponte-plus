...
export function SecretPinModal({ isOpen, onClose, onSuccess }: SecretPinModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div
        className={`relative w-full max-w-sm rounded-3xl p-6 bg-slate-900 border border-slate-700 shadow-2xl text-white space-y-5 transition-transform ${
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            {errorMsg && (
              <p className="text-center text-xs font-bold text-rose-400 mt-2 flex items-center justify-center gap-1">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={onClose}
              className="w-1/2 py-2.5 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-300 transition-all cursor-pointer"
            >
              Cancelar
            </button>
            <button
              ...
              className="w-1/2 py-2.5 rounded-xl text-xs font-bold bg-amber-500 hover:bg-amber-400 text-slate-950 transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shadow-lg shadow-amber-500/20"
            >
              Desbloquear
            </button>
          </div>
